...
5. **Sync at session end** - Run ` + "`git push`" + ` before ending your session
...
### Syncing
` + "```bash" + `
git push  # ALWAYS run at session end - pushes changes including beads data
` + "```" + `
...
- ✅ Run ` + "`git push`" + ` at end of session
